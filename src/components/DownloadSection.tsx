...
const VideoPreview: React.FC<{ 
  ...
  const handleDownload = async (formatId: string) => {
    ...
    try {
        // Extract quality from formatId (e.g., "1080p" -> 1080)
        const quality = parseInt(formatId.replace(/[^0-9]/g, ''));
        ...
        console.log('Starting download with params:', downloadParams);

        // Start progress polling
        const progressInterval = setInterval(async () => {
            try {
                const progressResponse = await fetch(`${API_ENDPOINTS.downloadProgress}/${downloadParams.filename}`);
        ...
        // Check if the response is JSON (error) or binary (video)
        ...
        let fileNameFromHeader = downloadParams.filename;
        ...
        if (err instanceof Error) {
            ...
            if (errorMessage.includes('Failed to download video after 3 attempts')) {
                errorMessage = 'Unable to download video. Please try again later or check if the video is available.';
            } else if (errorMessage.includes('Could not extract video information')) {
                errorMessage = 'Unable to access video information. The video might be private or no longer available.';
            } else if (errorMessage.includes('Downloaded file is empty')) {
                errorMessage = 'Download failed. The video might be unavailable or restricted.';
            }
...
const DownloadSection: React.FC = () => {
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
        console.log('Sending request to:', API_ENDPOINTS.info);
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.detail || `Failed to fetch video info: ${response.status}`);
        }
